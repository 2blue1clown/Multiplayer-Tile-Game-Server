...
class GameMaster():
    ...
    def ready_to_start(self):
        ...
        participants = len(inputs)-1
        if participants >= PLAYER_LIMIT:
            return True

        current_time = time.perf_counter()
        time_alive = current_time - self.born_time
        # Game should only start if its been going for 5 seconds to allow for
        # all initial clients to connect
        if time_alive < 5:
            return False

        if participants >= 2 and participants <= PLAYER_LIMIT:
